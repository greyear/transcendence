...
> Note: in this repository `auth-service` and `notification-service` run through Docker and are started with `make up`.
...
- **Notifications**: Stay updated on new followers, comments, and other interactions
...
- **Containerization**: Fully containerised deployment using Docker, orchestrated with Kubernetes
...
* The notification service stores delivery state and event tracking in PostgreSQL.
...
## MICROSERVICES:
The system is divided into microservices based on clearly defined responsibilities: authentication, core domain logic, notifications, and request routing.
...
## KUBERNETES:
Kubernetes (K8s) is an open-source container orchestration platform designed to manage, scale, and maintain containerised applications.

While Docker Compose is suitable for local development and simpler setups, it does not provide the orchestration, scalability, or fault tolerance needed for a microservices-based architecture. Kubernetes provides those capabilities by managing service lifecycle, networking, scaling, and resilience.

For this reason, the project is deployed with Kubernetes and tested locally with Minikube.
...
This gives a deployment model that closely resembles production-grade environments.
...
Minikube is a local Kubernetes distribution that runs a single-node cluster, where both control-plane and worker components operate on the same node. It is well-suited to development and educational use while preserving real Kubernetes behaviour.
