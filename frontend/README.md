...
- **[SCSS (Dart Sass)](https://sass-lang.com/)** — stylesheets live under [app/assets/styles](app/assets/styles) and are imported per component. Shared mixins and breakpoint variables are in `_mixins.scss` / `_variables.scss`.
...
- **`backdrop-filter`** (e.g. modal blur in [modal.scss](app/assets/styles/modal.scss)) is not supported — modals fall back to a plain semi-transparent overlay without blur.
- **`:has()` selector** (used in [languageSelector.scss](app/assets/styles/languageSelector.scss), [recipeCard.scss](app/assets/styles/recipeCard.scss), [userCard.scss](app/assets/styles/userCard.scss)) is only supported in Firefox 121+. On older versions, dependent styling (active language indicator, favorite-button hover states) will not apply.
- **`::-webkit-scrollbar`** styling in [cooksRow.scss](app/assets/styles/cooksRow.scss) is ignored; scrollbars use the native Firefox appearance. `scrollbar-width: none` is used as a fallback to hide them.
- **`-webkit-line-clamp`** truncation on recipe cards ([recipeCard.scss](app/assets/styles/recipeCard.scss)) may not clamp text; long titles/descriptions can overflow.
...
- **`:has()` selector** requires Safari 15.4+ (macOS 12.3+ / iOS 15.4+). Older versions lose the same styling listed for Firefox above.
